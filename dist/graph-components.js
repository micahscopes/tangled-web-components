(function () {
	...
	var graphEdgesPoly = {
	  ...
	  redraw: function redraw(elem) {
	    ...
	    edgeLines.enter().append("path").classed('edge', true).style('marker-start', 'url(#start-arrow)').style('marker-end', 'url(#end-arrow)');

	    shadowSVGSelector(elem).selectAll('.edge').attr("d", this.svgEdge).style('marker-start', 'url(#start-arrow)').style('marker-end', 'url(#end-arrow)');
	  },
	  ...
	  attached: function attached(elem) {
	    ...
	    setTimeout(function () {
	      setupArrows(shadowSVGSelector(elem));
	    }, 10);
	  },
